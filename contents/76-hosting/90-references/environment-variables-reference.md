...
WEB_SESSION_SECRET=c04d38a1-2ede-4a14-b306-8a0889236b43
...
# (boolean) Enable the `LIFECYCLE_POLICY` scheduled job. Defaults to `true`
SCHEDULER_MICROSOFT_TEAMS_LIFECYCLE_POLICY_ENABLED=true
# (string) Job schedule using the CRON syntax (See https://en.wikipedia.org/wiki/Cron). Defaults to `0 9 * * 0` (At 09:00 on every Sunday).
SCHEDULER_MICROSOFT_TEAMS_LIFECYCLE_POLICY_SCHEDULE=0 9 * * 0
...
# (string) Connected apps tray embedded root URL. Defaults to "https://tray.io/graphql".
CONNECTEDAPPS_TRAY_URL=https://tray.io/graphql
# (string) Connected apps tray embedded master token
CONNECTEDAPPS_TRAY_TOKEN=
...
# (string) Specify ssl policy. Defaults to `require`.
AUDITING_SERVICE_DB_AUDIT_SSL_MODE=require
...
# (boolean) Enable integration with the analytics service. Defaults to `true`.
ANALYTICS_SERVICE_INTEGRATION_ENABLED=true
# (string) analytics service URL. Defaults to `http://localhost:5001`.
ANALYTICS_SERVICE_URL=http://localhost:5001
# (string) URL of the analytics portal. Defaults to `http://localhost:3000/analytics`.
ANALYTICS_SERVICE_PORTAL=http://localhost:3000/analytics
# (string) An UUID token used to signify that an instance has permissions to create the initial User. This is created upon the first launch of Metabase, by the first instance; once used, it is cleared out, never to be used again. Defaults to ``.
ANALYTICS_SETUP_TOKEN=
# (string) When set, this will encrypt database credentials stored in the application database. Requirement: minimum 16 characters base64-encoded string. Defaults to ``.
ANALYTICS_ENCRYPTION_SECRET=
# (string) analytics service server secret used to sign embedded reports JWT tokens. Defaults to ``.
ANALYTICS_EMBEDDING_SECRET_KEY=
# (string) Login of the user used to access the list of available analytics. Defaults to ``.
ANALYTICS_SERVICE_USER=
# (string) Password of the user used to access the list of available analytics. Defaults to ``.
ANALYTICS_SERVICE_PASSWORD=
# (string) ID of the collection owning the available analytics. Defaults to ``.
ANALYTICS_SERVICE_ROOT_COLLECTION_ID=
...
# (string) HTTP user agent passed as a header with each request. Defaults to `nBold-Webhook/v5.2.0`.
WEBHOOKS_USER_AGENT=nBold-Webhook/v5.2.0
