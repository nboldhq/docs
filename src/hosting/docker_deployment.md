...
:::warning Warning
Hosting SalesTim on a docker server is for educational purpose only and is not considered as a supported production environment.
:::
...
After cloning our [Hosting](https://github.com/SalesTim/hosting) repository, open the docker project directory:
```bash
cd ./terraform/docker
```
